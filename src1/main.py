...
from src1.crop_seat_roi import build_roi_crop
...
def build_preview_with_method(
    ...
    if boundary_method == 1:
        from src1.find_boundary_hole1 import build_pattern_preview_from_centers
    ...
    if boundary_method == 2:
        from src1.find_boundary_hole2 import build_boundary_line_mask_from_centers
...
def process_path(
    ...
    scale = 1    # 0.5=缩小一半，可改成 0.4 / 0.3 / 0.2
    ...
    # 根据参数动态选择打孔提取的方法
    if extract_method == 1:
        from src1.extract_holes1 import build_hole_mask_and_centers
    elif extract_method == 2:
        from src1.extract_holes2 import build_hole_mask_and_centers
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=(1, 2),
        default=2,
        help="Hole extraction method: 1 (standard blackhat), 2 (adaptive threshold for stitching noise).",
    )
    parser.add_argument(
        "--boundary-method",
        type=int,
